...
<div class="card card-outline card-primary">
    ...
    <div class="card-body">
        ...
        @else
        <form method="POST" action="{{ url('/stok/'.$stocks->stok_id) }}" class="form-horizontal">
            @csrf
                ...
                <div class="col-11">
                    <select class="form-control" id="barang_id" name="barang_id" required>
                        ...
                        @foreach ($items as $item)
                            <option value="{{ $item->barang_id }}" @if ($item->barang_id == $stocks->barang_id)
                                selected
                ...
                <div class="col-11">
                    <select class="form-control" id="user_id" name="user_id" required>
                        ...
                        @foreach ($users as $item)
                            <option value="{{ $item->user_id }}" @if ($item->user_id == $stocks->user_id)
                                selected
            ...
            <div class="form-group row">
                ...
                <div class="col-11">
                    <input type="text" class="form-control" id="stok_jumlah" name="stok_jumlah" value="{{ old('stok_jumlah', $stocks->stok_jumlah) }}" required>
                    @error('stok_jumlah')
                        <small class="form-text text-danger">{{ $message }}</small>
                    @else
